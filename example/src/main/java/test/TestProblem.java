...
public class TestProblem {
	...
	public TestProblem(int load, double interactions, long seed, AgentsPolicy policy, int ticks, int agents) {
	...
	public void init(boolean warmup){
		TestModel model = new TestModel();
		
		sim.registerModel(model);
		
		if(warmup) sim.configureWithWarmup();
		else sim.configure();
