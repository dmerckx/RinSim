...
public class PDPTest {
    ...
    public void setUp() {
        ...
        obs = new Observer();
        pdpModel = new PdpModel(new LiberalPolicy(), obs);
    ...
    public void testStatesAtPickupForTruck() {
        ...
        PickupPoint<?> ppOutside = pp;             //The view of this pickup point to external agents
        
        TestDeliveryPoint dp = new TestDeliveryPoint(); //The actual delivery point
        DeliveryPoint<?> dpOutside = dp;             //The view of this pickup point to external agents
        
        //Note that the external view is captured at the start of a turn and is immutable thereafter.
        //The actual view COULD provide real-time date information to the actual agent.
        //In the case of a truck it does, in the case of a pickup/delivery-point it does not.
        
        sim.registerUser(t, new DummyData(pos));
        sim.registerUser(pp, new PickupPointData.Std(parcel));
        sim.registerUser(dp, new DeliveryPointData.Std(parcel));
        
        //During this test we will monitor the internal and external state of the 2 involved agents
        //For a truck these will differ:
        //  * the internal state captures the state AFTER a turn (since we don't monitor during turns)
        ...
        for(int v:getViews(1, 10)){
            ...
            if(v <= 4){
                ...
                //No packages picked up yet
                assertEquals(0, obs.packagesPickedUp);
            }
            else if(v == 5){
                ...
                //One package was picked up but this will be not be reflected until the PDP model was processed,
                //which happens at the start of each tick
                assertEquals(0, obs.packagesPickedUp);
            }
            else if(v == 6){
                ...
                //One package was picked up
                assertEquals(1, obs.packagesPickedUp);
            }
            else {
...
class TestDeliveryPoint extends DeliveryPoint.Std {
...
class Observer implements PdpObserver{
    public int packagesPickedUp = 0;
